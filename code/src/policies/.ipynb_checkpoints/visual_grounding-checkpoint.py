...
def iterative_segmentation_from_heatmap(
    ...
    sam_endpoint: str = "http://127.0.0.1:8001/sam2/point_predict"
) -> List[Dict[str, Any]]:
    """
    ...
    while initial_points:
        ...
        if mask_np is None or mask_np.size == 0:
            # print(f"[!] 警告: 点 {current_point} 未能生成有效掩码，跳过。")
            continue
...
def grounding(img: Image,
    ...
    # gen heatmap
    resized_img, heatmap = get_heatmap(base64_str, q, endpoint = "http://localhost:8101/attention_map")
    # get control points
