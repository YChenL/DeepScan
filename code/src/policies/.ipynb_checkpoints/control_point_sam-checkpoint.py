...
def sample_points_from_polygon(polygon: np.ndarray, num_points: int) -> List[Tuple[int, int]]:
    ...
    Args:
        polygon (np.ndarray): 多边形的顶点，形状为 (N, 1, 2)。
        num_points (int): 希望采样的点的数量。

    Returns:
        List[Tuple[int, int]]: 采样出的点的 (x, y) 坐标列表。
    """
    if num_points <= 0:
        return []
...
# if __name__ == '__main__':
    # # --- 1. 准备模拟输入 ---
    # # 创建一个模拟的输入图像 (PIL Image)
    # mock_image_pil = Image.new('RGB', (800, 600), 'darkgray')
    
    # # 将 PIL Image 转换为 OpenCV 使用的 BGR ndarray
    # mock_image_bgr = cv2.cvtColor(np.array(mock_image_pil), cv2.COLOR_RGB2BGR)

    # # 创建一个模拟的 heatmap (np.ndarray, float32)
    # # 模拟两个高亮区域
    # mock_heatmap = np.zeros((600, 800), dtype=np.float32)
    # # 使用高斯模糊创建平滑的热点
    # mock_heatmap[200, 250] = 1.0 # 物体1
    # mock_heatmap[400, 600] = 1.0 # 物体2
    # mock_heatmap[220, 280] = 0.8 # 物体1的另一个热点
    # mock_heatmap = cv2.GaussianBlur(mock_heatmap, (0, 0), sigmaX=30, sigmaY=30)
    
    
    # # --- 2. 执行核心算法 ---
    # # 步骤一：从热力图获取控制点
    # initial_control_points, binary_mask = filter_heatmap_and_find_centroids(mock_heatmap, min_area_threshold=100)
    
    # # 步骤二：迭代提取物体
    # segmented_objects = iterative_object_extraction(mock_image_bgr, initial_control_points)

    # # --- 3. 可视化结果 ---
    # plt.style.use('seaborn-v0_8-whitegrid')
    # fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    # fig.suptitle("物体提取与分割流程可视化", fontsize=20)
    
    # # 图1: 原始热力图
    # ax = axes[0, 0]
    # im = ax.imshow(mock_heatmap, cmap='hot')
    # ax.set_title("1. 输入的原始热力图 (Heatmap)")
    # fig.colorbar(im, ax=ax)

    # # 图2: 自适应阈值后的二值图和控制点
    # ax = axes[0, 1]
    # ax.imshow(binary_mask, cmap='gray')
    # ax.set_title("2. 自适应阈值后的高亮区域")
    # # 绘制控制点
    # if initial_control_points:
    #     points_x, points_y = zip(*initial_control_points)
    #     ax.scatter(points_x, points_y, c='red', s=50, marker='x', label='提取的控制点')
    #     ax.legend()

    # # 图3: 最终分割结果
    # ax = axes[1, 0]
    # ax.imshow(cv2.cvtColor(mock_image_bgr, cv2.COLOR_BGR2RGB))
    # ax.set_title(f"3. 最终分割结果 ({len(segmented_objects)} 个物体)")
    # colors = plt.cm.get_cmap('tab10', len(segmented_objects)) # 为每个物体生成不同颜色
    
    # for i, obj in enumerate(segmented_objects):
    #     mask = obj['mask']
    #     source_point = obj['source_point']
        
    #     # 创建一个彩色的叠加层
    #     color_mask = np.zeros_like(mock_image_bgr)
    #     color_mask[mask == 255] = [int(c * 255) for c in colors(i)[:3][::-1]] # Matplotlib RGB to BGR
        
    #     # 将叠加层与原图混合
    #     overlay = cv2.addWeighted(mock_image_bgr, 1, color_mask, 0.6, 0)
    #     mock_image_bgr = overlay # 更新图像以便下一次叠加
        
    #     # 标记每个物体的源控制点
    #     cv2.circle(mock_image_bgr, source_point, 10, [int(c * 255) for c in colors(i)[:3][::-1]], -1)
    #     cv2.putText(mock_image_bgr, f"Obj {i+1}", (source_point[0] + 15, source_point[1] + 5), 
